...
public record RecipeIngredientPantryMatchResponseDto(
        ...
        @Schema(
                        description = "보유 여부. 요리가능 ON인 매칭 항목이 1개 이상이면 true (요리가능 OFF 항목만 있으면 false)",
                        requiredMode = Schema.RequiredMode.REQUIRED)
                boolean hasIngredient,
        @Schema(description = "동일 식재료 ID로 매칭되는 팬트리 항목. 요리가능 여부와 무관하게 모두 포함(조리완료 정리 대상 용도)")
    ...
    public record MatchedPantryItemResponseDto(
            ...
            @Schema(description = "요리가능 여부", requiredMode = Schema.RequiredMode.REQUIRED) boolean cookable) {
