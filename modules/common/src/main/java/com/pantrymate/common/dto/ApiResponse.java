...
public record ApiResponse<T>(
    ...
    Instant timestamp    // UTC, ISO-8601 ("...Z")
) {
    ...
    public static <T> ApiResponse<T> error(String message, String errorCode) {
        return new ApiResponse<>("ERROR", message, null, errorCode, Instant.now());
    }

    public static ApiResponse<Void> error(ErrorCode errorCode) {
        return new ApiResponse<>("ERROR", errorCode.getMessage(), null, errorCode.getCode(), Instant.now());
    }
}
